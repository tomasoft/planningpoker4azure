...
using PlanningPoker.Domain.Test;
using PlanningPoker.Service;
using Deck = PlanningPoker.Service.Deck;
using Estimation = PlanningPoker.Domain.Estimation;
using EstimationResultMessage = PlanningPoker.Service.EstimationResultMessage;
using EstimationSetMessage = PlanningPoker.Service.EstimationSetMessage;
using MemberMessage = PlanningPoker.Service.MemberMessage;
using Message = PlanningPoker.Domain.Message;
using MessageType = PlanningPoker.Service.MessageType;
using ScrumTeam = PlanningPoker.Domain.ScrumTeam;
using TeamState = PlanningPoker.Domain.TeamState;
using TimerMessage = PlanningPoker.Service.TimerMessage;

namespace PlanningPoker.Test.Service
{
    [TestClass]
    public class PlanningPokerServiceTest
    {
        ...
        public void Constructor_PlanningPoker_PlanningPokerPropertyIsSet()
        {
            ...
            // Verify
            Assert.AreEqual<IPlanningPoker>(planningPoker.Object, result.PlanningPoker);
        }
        ...
        [DataTestMethod]
        [DataRow(Deck.Standard, Domain.Deck.Standard)]
        [DataRow(Deck.Fibonacci, Domain.Deck.Fibonacci)]
        [DataRow(Deck.RockPaperScissorsLizardSpock, Domain.Deck.RockPaperScissorsLizardSpock)]
        public void CreateTeam_TeamNameAndScrumMasterName_ReturnsCreatedTeam(Deck deck, Domain.Deck domainDeck)
        {
            // Arrange
            ...
            Assert.AreEqual<string>(typeof(ScrumMaster).Name, resultTeam.ScrumMaster.Type);
        }
        ...
        public void JoinTeam_TeamNameAndMemberNameAsMember_ReturnsTeamJoined()
        {
            ...
            var expectedMemberTypes = new string[] { typeof(ScrumMaster).Name, typeof(Member).Name };
        ...
        public void JoinTeam_TeamNameAndObserverNameAsObserver_ReturnsTeamJoined()
        {
            ...
            var expectedMemberTypes = new string[] { typeof(Observer).Name };
        ...
        public void ReconnectTeam_TeamNameAndScrumMasterName_ReturnsReconnectedTeam()
        {
            ...
            var expectedMemberTypes = new string[] { typeof(ScrumMaster).Name };
        ...
        public void ReconnectTeam_TeamNameAndMemberNameAndEstimationInProgress_NoEstimationResult()
        {
            ...
            team.ScrumMaster!.StartEstimation();
            member.Estimation = new Estimation(1);
        ...
        public void ReconnectTeam_TeamNameAndMemberNameAndEstimationFinished_EstimationResultIsSet()
        {
            ...
            team.ScrumMaster!.StartEstimation();
            member.Estimation = new Estimation(1);
            team.ScrumMaster.Estimation = new Estimation(2);
        ...
        public void ChangeDeck_TeamNameAndFibonacci_AvailableEstimationsAreChanged()
        {
            ...
            var fibonacciDeck = DeckProvider.Default.GetDeck(Domain.Deck.Fibonacci);
        ...
        public async Task GetMessages_EstimationEnded_ScrumMasterGetsMessages()
        {
            ...
            master.Estimation = new Estimation(1.0);
            member.Estimation = new Estimation(2.0);
        ...
        public async Task GetMessages_AvailableEstimationsChanged_MemberGetsMessages()
        {
            ...
            var team = CreateBasicTeam(new GuidProviderMock(guid));
            var member = (Member)team.Join(MemberName, false);
            var deck = DeckProvider.Default.GetDeck(Domain.Deck.Fibonacci);
            ...
            Assert.AreEqual(PlanningPoker.Service.Estimation.PositiveInfinity, estimations[11].Value);
        ...
        public async Task GetMessages_NoMessagesOnTime_ReturnsEmptyCollection()
        {
            ...
            planningPoker.Setup(p => p.GetScrumTeam(TeamName)).Returns(teamLock.Object).Verifiable();
            planningPoker.Setup(p => p.GetMessagesAsync(team.ScrumMaster!, It.IsAny<CancellationToken>()))
                .ReturnsAsync(Enumerable.Empty<Message>()).Verifiable();
            var target = CreatePlanningPokerService(planningPoker.Object);
        ...
        private static ScrumTeam CreateBasicTeam(GuidProvider? guidProvider = null, DateTimeProvider? dateTimeProvider = null)
        {
            var result = new ScrumTeam(TeamName, null, dateTimeProvider, guidProvider);
        ...
        private static PlanningPokerService CreatePlanningPokerService(IPlanningPoker planningPoker, DateTimeProvider? dateTimeProvider = null)
        {
            ...
            return new PlanningPokerService(planningPoker, dateTimeProvider, DeckProvider.Default);
        }
    }
}
