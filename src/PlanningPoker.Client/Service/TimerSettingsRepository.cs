...
namespace PlanningPoker.Client.Service
{
    ...
    public class TimerSettingsRepository : ITimerSettingsRepository
    {
        ...
        public async Task<TimeSpan?> GetTimerDurationAsync()
        {
            try
            {
                var timerDurationString = await _jsRuntime.InvokeAsync<string?>("Ploker.PlanningPoker.getTimerDuration");
        ...
        public async Task SetTimerDurationAsync(TimeSpan timerDuration)
        {
            try
            {
                ...
                await _jsRuntime.InvokeVoidAsync("Ploker.PlanningPoker.setTimerDuration", timerDurationString);
            }
            catch (Exception)
            {
