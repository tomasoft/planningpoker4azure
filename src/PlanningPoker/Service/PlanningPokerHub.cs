...
namespace PlanningPoker.Service
{
    ...
    public class PlanningPokerHub : Hub<IPlanningPokerClient>
    {
        ...
        /// </summary>
        public IPlanningPoker PlanningPoker { get; }
        ...
        public TeamResult CreateTeam(string teamName, string scrumMasterName, Deck deck)
        {
            ...
            try
            {
                ...
                {
                    teamLock.Lock();
                    var resultTeam = ServiceEntityMapper.Map<Domain.ScrumTeam, ScrumTeam>(teamLock.Team);
        ...
        public ReconnectTeamResult ReconnectTeam(string teamName, string memberName)
        {
            ...
            try
            {
                using (var teamLock = PlanningPoker.GetScrumTeam(teamName))
                {
                    ...
                    if (team.State == Domain.TeamState.EstimationInProgress && observer is Member member)
                    {
                        selectedEstimation = ServiceEntityMapper.Map<Domain.Estimation?, Estimation?>(member.Estimation);
                    }
        ...
        public void SubmitEstimation(string teamName, string memberName, double? estimation)
        {
            ...
            using (var teamLock = PlanningPoker.GetScrumTeam(teamName))
            {
                ...
                if (member != null)
                {
                    member.Estimation = new Domain.Estimation(estimation);
                }
            }
        }
        ...
        public void GetMessages(string teamName, string memberName, Guid sessionId, long lastMessageId)
        {
            ...
            Task<IEnumerable<Domain.Message>> receiveMessagesTask;
        ...
        private async void OnMessageReceived(Task<IEnumerable<Domain.Message>> receiveMessagesTask, string connectionId)
        {
            try
            {
                var messages = await receiveMessagesTask;
                var clientMessages = messages.Select(ServiceEntityMapper.FilterMessage)
                    .Select(ServiceEntityMapper.Map<Domain.Message, Message>).ToList();
