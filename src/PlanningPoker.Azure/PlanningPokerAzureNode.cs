...
using PlanningPoker.Azure.Configuration;
using PlanningPoker.Azure.ServiceBus;
using PlanningPoker.Domain;
using PlanningPoker.Domain.Serialization;

namespace PlanningPoker.Azure
{
    /// <summary>
    ...
    public class PlanningPokerAzureNode : IDisposable
    {
        ...
        private static readonly byte[] _deletedTeamPrefixBytes = Encoding.UTF8.GetBytes(DeletedTeamPrefix);
        ...
        {
            if (data.Length > _deletedTeamPrefixBytes.Length)
            {
                for (int i = 0; i < _deletedTeamPrefixBytes.Length; i++)
                {
                    if (data[i] != _deletedTeamPrefixBytes[i])
                    {
                        return null;
                    }
                }

                return Encoding.UTF8.GetString(data, _deletedTeamPrefixBytes.Length, data.Length - _deletedTeamPrefixBytes.Length);
            }
